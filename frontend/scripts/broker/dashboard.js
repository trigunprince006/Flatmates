async function logout() {
  const response = await fetch("http://localhost:4000/broker/logout", {
    method: "POST",
    credentials: "include",
  });
...
async function loadProfile() {
  const response = await fetch("http://localhost:4000/broker/profile", {
    credentials: "include",
  });

  const data = await response.json();
  console.log(data);
  document.getElementById("my-listed-property").innerText =
    data.totalListedProperty;
  if (data.success === false) {
    const response = await fetch("http://localhost:4000/broker/refresh-token", {
      method: "POST",
      credentials: "include",
    });
  }

  document.getElementById("username").innerHTML = data.broker.fullname;
  // console.log(data);
  document.getElementById("image").src = data.broker.userProfile.profilePhoto;
}
loadProfile();
